...
`sharding_indexed`, and `transpose`, are compiled in. `blosc` is enabled by
default on non-Windows builds and disabled by default on Windows because the
vendored C-Blosc pthread shims conflict with Rtools' winpthread at DLL link time;
use `codec_capabilities()` or `arr$codec_capabilities()` to inspect support.
