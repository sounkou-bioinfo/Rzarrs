...
Current ZIP support is intentionally reader-first and local-file oriented: the
archive is loaded into a Rust memory store before opening the Zarr hierarchy.
That is useful for small VCF fixtures and portable examples. Large production
VCF Zarr archives should be stored as normal directory/object-store Zarr, or the
backend should be switched to upstream `zarrs_zip` once it is vendored for this
package. `zarrs_zip` is the right long-term target because it can provide a real
Zarr storage adapter instead of R-level or ad hoc extraction.
